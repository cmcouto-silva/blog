...
Post atualizado pela última vez em: ```r format(Sys.time(), "%d de %B de %Y, %H:%M (GMT+1)")```.
...
plt_lst <- lapply(target_countries, function(country) {
  ...
  g <- ggplot(dt[Localização==country], aes(Data, Quantidade, color=Categoria)) +
    ...
    labs(x="", y="",color="Legenda", title=paste("Incidência de CODIV-19", prep_country)) +
...
top10 <- ggplot(top10, aes(Data, Quantidade, color=Categoria, fill=Localização)) +
  ...
  labs(x="", y="", fill='País',
       title="Países com maiores incidências de CODVID-19") +
